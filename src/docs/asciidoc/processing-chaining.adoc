...
image::use-cases-processing.png[width=1000,align="center"]
...
image::processing-overview.png[width=1000,align="center"]
...
image::processing-application-descriptor.png[width=1000,align="center"]
...
.EMS Deployment
image::processing-EMS-deploy.png[width=1000,align="center"]
...
image::processing-EMS-execute.png[width=1000,align="center"]
...
<<procK8s>> introduces the use of Kubernetes (K8s) as the provider agnostic interface to the Resource Layer. The ADES has touch-points with the Resource Layer for access to data and compute resource. The following sub-section elaborate the approach.
...
image::processing-ADES-execute.png[width=1000,align="center"]
...
Nevertheless, we might envisage that access to the underlying data will be provided by the hosting platform through a variety of data access protocols, including: Object Store (S3/Swift), OGC (WMS, WCS, WFS), OPeNDAP, plus local file-system as mentioned above.
...
Solutions to the above are explored in the following <<procDataAccess, section>>.

[[procDataAccess]]
=== Data Access

In cases where the processing service does not natively support the data access protocols offered by the underlying platform, the processing framework must facilitate acces to platform data in a form that can be consumed by the processing service.
...
See section <<resourcesDataAccessGateway>>.
...
include::wps-t-rest-interface.adoc[]
...
image::use-cases-applications.png[width=1000,align="center"]
...
[[procAppCatalogue,Application Catalogue]]
== Application Catalogue

The Application Catalogue provides an inventory of applications/services that can be presented as a ‘Marketplace’ for users to discover and browse for processing services to use with EP data.

The metadata for each application record describes what data an application can be applied to, and how it can be chained in a workflow.

The current current OGC Testbed-15 (EOPAD)

The Application Catalogue is the subject of the current OGC Testbed-15 EOPAD Thread, through which the Data Model and catalogue Service Interface are being explored:
