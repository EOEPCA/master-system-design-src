...
The main components comprising the Resource Management domain are illustrated in <<fimg_resourceOverview>>:
...
* OpenSearch GEO: OpenSearch Geo and Time Extensions <<OGC_10-032r8>>
* OpenSearch EO: OGC OpenSearch Extension for Earth Observation <<OGC_13-026r9>>
* OGC EO Dataset Metadata GeoJSON(-LD) Encoding Standard <<OGC_17-003r1_17-084>>
* OGC OpenSearch-EO GeoJSON(-LD) Response Encoding Standard <<OGC_17-047>>
...
Thus, the Data Access Gateway must implement the data access interface to the infrastructure Resource Tier storage, and present a standard internal interface to other EP components. The internal interface must be sufficient to support the needs of the EP components:

* Stage-in / out of processing service data
* Data respository for Data Access services (WMS, WCS, WFS, etc.)
* Data storage for ingestion of data

At minimum, it would be expected to support Resource Tier interfaces for:

* AWS S3 Object Store
* Swift Object Store (OpenStack)

The gateway provides a data bridge between the EP components and the Resource Tier. It fills the gap in the data access capabilities of a given a given service/application, and provides a common data access interface that such components can target in their implementation.

== Data Access Library

In addition to the Data Access Gateway, which operates as an internal service, the Data Access Library is provided specifically as a point of integration for processing services and applications.

The Data Access Library provides:
...
Processing service/applications are implemented against an object model that is well defined within the bindings for their development language. The object model hides the details of the underlying files/storage. Thus, processing services and applications can be implemented in a ‘portable’ way that is agnostic to the platform resource-tier storage technology.

The library offered to the processing service at runtime must implement the interface to the data access to the resource-tier storage. Hence, the library should not be ‘hard-coded’ into the processor application package (Docker image), but should instead be provided dynamically by the platform processing framework.
